...
func dump(module common.Module) {
	...
	for i, code := range d.module.CodeSec {
		...
		if len(code.Locals) > 0 {
			for i, locals := range code.Locals {
				...
				fmt.Printf("%s x %d",
					common.ValTypeToStr(locals.Type), locals.N)
			}
		}
		fmt.Println("]")
